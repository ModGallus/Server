package org.areillan.game.world.map;

import org.areillan.game.content.skill.member.construction.BuildHotspot;
import org.areillan.game.content.skill.member.construction.BuildHotspotType;
import org.areillan.game.content.skill.member.construction.Hotspot;
import org.areillan.game.node.entity.player.Player;
import org.areillan.game.node.item.GroundItem;
import org.areillan.game.node.item.Item;
import org.areillan.game.node.object.Constructed;
import org.areillan.game.node.object.GameObject;
import org.areillan.game.world.map.build.LandscapeParser;
...
//			if (direction == RoomBuilder.DIRECTIONS[i]) {
//				rotation = i;
//				break;
//			}
//		}
	 ...
	 * @param hotspots the hotspots to be rotated
	 * @param direction the direction
	 * @return the modified hotspots
	 */
	public Hotspot[] rotateHotspots(Hotspot[] hotspots, Direction direction) {
		if (rotation != 0) {
			System.err.println("Region chunk was already rotated!");
			return hotspots;
		}
		...
		for (int i = 0; i < objects.length; i++) {
			for (int x = 0; x < SIZE; x++) {
				for (int y = 0; y < SIZE; y++) {
					...
					if (object != null) {
						int[] pos = getRotatedPosition(x, y, object.getDefinition().getSizeX(), object.getDefinition().getSizeY(), object.getRotation(), rotation);
						Location newLoc = object.getLocation().transform(pos[0] - x, pos[1] - y, 0);
						GameObject obj = object.transform(object.getId(), (object.getRotation() + rotation) % 4, newLoc);
						for (int j = 0; j < hotspots.length; j++) {
							Hotspot h = hotspots[j];
							if (h.getHotspot().getType() == BuildHotspotType.RECURSIVE || h.getHotspot() == BuildHotspot.WINDOW) 
								continue;
							if (h.getDecorationIndex() == -1) {
								if (h.getChunkX() == x && h.getChunkY() == y && h.getHotspot().getObjectId() == object.getId()) {
									h.setChunks(newLoc.getChunkOffsetX(), newLoc.getChunkOffsetY());
								} 
							} else if (h.getChunkX() == x && h.getChunkY() == y && h.getHotspot().getDecorations()[h.getDecorationIndex()].getObjectId() == object.getId()) {
								h.setChunks(newLoc.getChunkOffsetX(), newLoc.getChunkOffsetY());
							}
						}
						if (object instanceof Constructed) {
