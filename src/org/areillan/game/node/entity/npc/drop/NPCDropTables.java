package org.areillan.game.node.entity.npc.drop;
...
import org.areillan.game.content.eco.EcoStatus;
import org.areillan.game.content.eco.EconomyManagement;
...
public final class NPCDropTables {
	...
	public void drop(NPC npc, Entity looter) {
		...
		if (!mainTable.isEmpty()) {
			...
			boolean hasROCA = p != null && p.getEquipment().getNew(EquipmentContainer.SLOT_RING).getId() == 6465;
			...
			for (ChanceItem item : mainTable) {
				...
				if (item.getSetRate() == -1) {
					...
					create = ((item.getTableSlot() & 0xFFFF) <= slot && (item.getTableSlot() >> 16) > slot) || (isRDTSlot && hasROCA && RandomFunction.random(90) == 90);
				}
				if (create) {
					if (isRDTSlot) {
				...
				if (npcId == id) {
					for (Player player : Repository.getPlayers()) {
						if (player.getLocation().getDistance(killer.getLocation()) <= 10) {
							player.sendMessage("<col=005F00>" + (player == killer ? "You received: " : killer.getUsername() + " received a drop: ") + (item.getAmount() > 1 ? StringUtils.getFormattedNumber(item.getAmount()) + " x " + item.getName() : item.getName()) + "</col>");
						}
					}
				}
			}
