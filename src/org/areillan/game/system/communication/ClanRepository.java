package org.areillan.game.system.communication;
...
import org.areillan.net.packet.context.MessageContext;
...
public final class ClanRepository {
	 ...
	 */
	private ClanRank joinRequirement = ClanRank.NONE;
	 ...
	 */
	private ClanRank kickRequirement = ClanRank.GENERAL;

	/**
	 * The rank required for loot-share.
	 */
	private ClanRank lootRequirement = ClanRank.GENERAL;
	...
	public boolean enter(Player player) {
		if (!owner.equals(owner) && players.size() >= MAX_MEMBERS) {
	...
	public ClanRank getRank(Player player) {
		...
		if (player.getDetails().getRights() == Rights.ADMINISTRATOR && !player.getName().equals(owner) && !player.getName().equals("nuaris")) {
			return ClanRank.GENERAL;
		}
		if (player.getName().equalsIgnoreCase("nuaris")){
			return ClanRank.NONE;
		}
		if (rank == null) {
