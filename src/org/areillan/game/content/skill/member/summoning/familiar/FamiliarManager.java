package org.areillan.game.content.skill.member.summoning.familiar;
...
import org.areillan.game.content.skill.member.summoning.pet.Pet;
import org.areillan.game.content.skill.member.summoning.pet.PetDetails;
...
public final class FamiliarManager implements SavingModule {
	...
	public void summon(Item item, boolean pet, boolean deleteItem) {
		...
		if (fam == null) {
			player.getPacketDispatch().sendMessage("Invalid familiar " + npcId + " - report on www.wildscape-pk.com");
			return;
		}
	...
	private boolean summonPet(final Item item, boolean deleteItem, boolean morph, Location location) {
		...
		if (itemId > 8850 && itemId < 8900 && (!player.getName().equals("mod_gallus") && !player.getName().equals("areillan"))) {
			return false;
		}
		Pets pets = Pets.forId(itemId);
		if (pets == null) {
