...
public final class DSlayerDrainPlugin extends UseWithHandler {
	...
	public boolean handle(NodeUsageEvent event) {
		...
		if (player.getInventory().remove(BUCKET_OF_WATER)) {
			...
			if (quest.getStage(player) == 20 && !player.getInventory().containsItem(DemonSlayer.FIRST_KEY) && !player.getBank().containsItem(DemonSlayer.FIRST_KEY) && !player.getSavedData().getQuestData().getDemonSlayer()[0]) {
