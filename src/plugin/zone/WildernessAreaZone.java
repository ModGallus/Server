			...
			for (int itemId : ALLOWED) {
				if (event.getUsedItem().getId() == itemId) {
					...
					player.getDialogueInterpreter().addAction(new DialogueAction() {
						@Override
						public void handle(Player player, int buttonId) {
							if (player.getInventory().remove(new Item(995, price)) && player.getInventory().remove(new Item(event.getUsedItem().getId(), amount))) {
								player.getInventory().add(new Item(event.getUsedItem().getNoteChange(), amount));
								player.getDialogueInterpreter().sendItemMessage(event.getUsedItem().getId(), "Piles converts your items to banknotes.");
							}
						}
					});
					return true;
