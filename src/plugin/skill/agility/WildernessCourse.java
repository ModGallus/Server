...
import org.areillan.game.content.skill.member.agility.AgilityCourse;
...
public final class WildernessCourse extends AgilityCourse {
	...
	private void handleRockClimb(final Player player, final GameObject object) {
		...
		GameWorld.submit(new Pulse(4, player) {
			@Override
			public boolean pulse() {
				player.getDialogueInterpreter().sendItemMessage(GRACE.getId(), "You have received a Mark of Grace!");
				player.getInventory().add(GRACE);
