...
import org.areillan.game.content.skill.member.construction.Room;
...
public final class BuildRoomDialogue extends DialoguePlugin {
	 ...
	 */
	private static final int[][] BOUNDARY_OFFSETS = {
		{7, 4},
		{4, 0},
		{0, 4},
		{4, 7}
	...
	public boolean open(Object... args) {
		...
		if (GameWorld.getSettings().isDevMode()) {
			System.out.println("Room exits: " + Arrays.toString(this.exits));
		}
	...
	private void rotate(boolean counter) {
		...
		if (GameWorld.getSettings().isDevMode()) {
			System.out.println("Roar " + direction);
		}
	...
	private void drawBoundaries() {
		...
		for (int j = 0; j < exit.length; j++) {
			exit[j] = exits[(j + index) % exit.length];
		}
		if (GameWorld.getSettings().isDevMode()) {
