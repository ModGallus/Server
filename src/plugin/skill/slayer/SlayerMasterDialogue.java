...
import org.areillan.game.content.global.Skillcape;
...
public final class SlayerMasterDialogue extends DialoguePlugin {
	...
	public boolean open(Object... args) {
		...
		if (master == Master.SUMONA) {
			options("'Ello, and what are you after, then?", "Something else");
			stage = -1;
			return true;
		}
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 701:
			...
			if (!player.isAdmin() && Master.hasSameTask(master, player.getSlayer().getMaster(), player) && !player.getDetails().getShop().hasPerk(Perks.SLAYER_BETRAYER)) {
	...
	@Override
	public int[] getIds() {
		return new int[] { 70, 1598, 1596, 1597, 1599, 3820, 7780, 8275, 8273, 8274, 8649 };
	}
