...
import org.areillan.game.content.eco.ge.GEGuidePrice;
import org.areillan.game.content.eco.ge.GrandExchangeDatabase;
import org.areillan.game.content.eco.ge.GEGuidePrice.GuideType;
...
public final class GrandExchangePlugin extends OptionHandler {
	...
	public Plugin<Object> newInstance(Object arg) throws Throwable {
		...
		for (int i : new int[] { 6528, 6529, 6530, 6531 }) {
			NPCDefinition.forId(i).getConfigurations().put("option:talk-to", this);
	...
	public boolean isWalk(final Player player, final Node node) {
		if (node.getLocation().withinDistance(player.getLocation(), 2)) {
	...
	public Location getDestination(Node node, Node n) {
		if (n instanceof NPC) {
			return n.getLocation().transform(n.getLocation().getX() == 3166 ? 1 : -1, 0, 0);
	...
	public final class GENPCPlugin extends OptionHandler {
		...
		public Plugin<Object> newInstance(Object arg) throws Throwable {
			...
			NPCDefinition.forId(6533).getConfigurations().put("option:bank", this);
			...
			NPCDefinition.forId(6535).getConfigurations().put("option:collect", this);
			return this;
		}
		...
		public Location getDestination(Node node, Node n) {
			...
			if (n instanceof NPC) {
				//System.out.println("Banker: "+node.getDirection()+" Player: "+n.getDirection());
				return n.getLocation().transform(0, n.getLocation().getY() == 3488 ? -1 : 1, 0);
				/*switch (n.getDirection()) {
				case EAST:
					return n.getLocation().transform(-1, 0, 0);
				case NORTH:
				...
				case WEST:
					return n.getLocation().transform(1, 0, 0);
				case SOUTH_WEST:
					return n.getLocation().transform(1, -1, 0);
				case SOUTH_EAST:
					return n.getLocation().transform(-1, -1, 0);
				case NORTH_WEST:
					return n.getLocation().transform(1, 1, 0);
